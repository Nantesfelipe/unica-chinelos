import { Pencil, Trash2, Package, Images } from 'lucide-react';

import formatCurrency from '../../utils/formatCurrency';

function TabelaProdutos({ produtos, onEditar, onExcluir, onGerenciarMidia }) {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-[#8e8980] border-b border-[#8e8980]/20">
            ...
            <th className="px-5 py-3 font-normal">Destaque</th>
            <th className="px-5 py-3 font-normal">Promoção</th>
        ...
        <tbody className="divide-y divide-[#8e8980]/10">
          {produtos.map((produto) => (
            <tr key={produto.id}>
              ...
              <td className="px-5 py-3 text-[#8e8980]">{produto.destaque ? 'Sim' : 'Não'}</td>
              <td className="px-5 py-3 text-[#8e8980]">{produto.promocao ? 'Sim' : 'Não'}</td>
