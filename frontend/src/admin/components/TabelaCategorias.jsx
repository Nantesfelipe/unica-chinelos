...
function TabelaCategorias({ categorias, onExcluir }) {
  if (categorias.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-[#8e8980]/20 p-10 text-center">
        <Tags size={32} className="mx-auto text-[#8e8980]" />
        <p className="text-sm text-[#8e8980] mt-3">Nenhuma categoria cadastrada.</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-[#8e8980]/10">
          {categorias.map((categoria) => (
            <tr key={categoria.id}>
              <td className="px-5 py-3 text-[#171511]">{categoria.nome}</td>
